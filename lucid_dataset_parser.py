...
def parse_pcap_cached(pcap_path, in_labels, dataset_type, max_flow_len, max_flows=0, traffic_type='all', time_window=TIME_WINDOW, cache_dir="./cache/"):
            os.makedirs(cache_dir, exist_ok=True)

            base_filename = os.path.basename(pcap_path)
            cache_file = os.path.join(cache_dir, base_filename + ".joblib")
            meta_file = os.path.join(cache_dir, base_filename + ".meta")
            current_pcap_mtime = os.path.getmtime(pcap_path)

            if os.path.exists(cache_file) and os.path.exists(meta_file):
                with open(meta_file, "r") as f:
                    cached_mtime = float(f.read().strip())
                if abs(cached_mtime - current_pcap_mtime) < 1:
                    print(f"[Cache Hit] {pcap_path}")
                    return load(cache_file)
                else:
                    print(f"[Cache Stale] {pcap_path} changed. Re-parsing...")
            else:
                print(f"[Cache Miss] {pcap_path}")

            labelled_flows = []
            process_pcap(pcap_path, dataset_type, in_labels, max_flow_len, labelled_flows, max_flows, traffic_type, time_window)

            dump(labelled_flows, cache_file)
            with open(meta_file, "w") as f:
                f.write(str(current_pcap_mtime))

            return labelled_flows

def process_pcap_wrapper(args_tuple):
            pcap_path, in_labels, dataset_type, max_flow_len, max_flows, traffic_type, time_window = args_tuple
            return parse_pcap_cached(pcap_path, in_labels, dataset_type, max_flow_len, max_flows, traffic_type, time_window)
...
def main(argv):
    ...
    dataset_id = str(args.dataset_id[0]) if args.dataset_id else ''
    ...
    if args.dataset_folder is not None and args.dataset_type is not None:
        ...
        print(f"[INFO] Starting multiprocessing with {os.cpu_count()} cores...")
        start_time = time.time()

        args_list = [(file, in_labels, args.dataset_type[0], max_flow_len, args.max_flows, traffic_type, time_window) for file in filelist]

        with Pool(processes=os.cpu_count()) as pool:
            results = list(tqdm(pool.imap(process_pcap_wrapper, args_list), total=len(args_list)))

        elapsed_time = time.time() - start_time
        print(f"[INFO] Parsing completed in {elapsed_time:.2f} seconds.")

        preprocessed_flows = []
        for labelled_flows in results:
            preprocessed_flows.extend(labelled_flows)

        print(f"[INFO] Total parsed flows: {len(preprocessed_flows)}")

        # Save preprocessed flows
        process_time = time.time() - start_time
        ...
        filename = str(int(time_window)) + 't-' + str(max_flow_len) + 'n-' + dataset_id + '-preprocess'
        output_file = output_folder + '/' + filename
        output_file = output_file.replace("//", "/")  # remove double slashes if needed
